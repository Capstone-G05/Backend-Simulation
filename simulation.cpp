...
int main() {
    ...
    int rc;
    rc = getDataFromRedis("127.0.0.1", 6379, "auger_bottom_pivot_angle_max", redisValue, "");
    if (rc == 0 && !redisValue.empty()) augerBottomPivotAngleMax = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "auger_bottom_pivot_angle_min", redisValue, "");
    if (rc == 0 && !redisValue.empty()) augerBottomPivotAngleMin = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "auger_bottom_pivot_speed_ref", redisValue, "");
    if (rc == 0 && !redisValue.empty()) augerBottomPivotSpeedRef = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "auger_top_angle_min", redisValue, "");
    if (rc == 0 && !redisValue.empty()) augerTopAngleMin = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "auger_top_angle_max", redisValue, "");
    if (rc == 0 && !redisValue.empty()) augerTopAngleMax = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "auger_top_speed_ref", redisValue, "");
    if (rc == 0 && !redisValue.empty()) augerTopRefSpeed = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "spout_tilt_angle_min", redisValue, "");
    if (rc == 0 && !redisValue.empty()) spoutTiltAngleMin = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "spout_tilt_angle_max", redisValue, "");
    if (rc == 0 && !redisValue.empty()) spoutTiltAngleMax = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "spout_tilt_speed_ref", redisValue, "");
    if (rc == 0 && !redisValue.empty()) spoutTiltRefSpeed = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "head_rotation_angle_min", redisValue, "");
    if (rc == 0 && !redisValue.empty()) headRotationAngleMin = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "head_rotation_angle_max", redisValue, "");
    if (rc == 0 && !redisValue.empty()) headRotationAngleMax = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "head_rotation_speed_ref", redisValue, "");
    if (rc == 0 && !redisValue.empty()) headRotationRefSpeed = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "gate_angle_min", redisValue, "");
    if (rc == 0 && !redisValue.empty()) gateAngleMin = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "gate_angle_max", redisValue, "");
    if (rc == 0 && !redisValue.empty()) gateAngleMax = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "gate_speed_ref", redisValue, "");
    if (rc == 0 && !redisValue.empty()) gateRefSpeed = std::atoi(redisValue.c_str());
    
    rc = getDataFromRedis("127.0.0.1", 6379, "machine_type", redisValue, "");
    ...
    while (true) {
        ...
        if (redisCheckCounter >= 10) {
            ...
            rc = getDataFromRedis("127.0.0.1", 6379, "pto", redisValue, "");
            if (rc == 0 && !redisValue.empty()) {
            ...
            rc = getDataFromRedis("127.0.0.1", 6379, "crop_fill_rate", redisValue, "");
            if (rc == 0 && !redisValue.empty()) {
